  ...
  return (
    <div  className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-md w-full max-w-sm">
        <h2 className="text-2xl font-bold mb-6 text-center">Inscrivez-vous en tant que Admin</h2>

      
        <div className="mb-6">
          <label htmlFor="nom" className="block text-gray-700 mb-1">Nom</label>
          <input type="text" name="nom" id="nom" value={formulaire.nom}  onChange={handleChange} className="w-full border-b-2 border-gray-300 focus:outline-none focus:border-blue-500 py-2 px-1 bg-transparent"
            placeholder="Ex: Jonh Doe"
          />
          {messagePartiel.nom && <span className="text-red-500 text-sm">{messagePartiel.nom}</span>}
        </div>
        ...
        <div className="mb-6">
          ...
          <input type="text"  name="email" id="email"  value={formulaire.email} onChange={handleChange} className="w-full border-b-2 border-gray-300 focus:outline-none focus:border-blue-500 py-2 px-1 bg-transparent"
            ...
            {messagePartiel.email && <span className="text-red-500 text-sm">{messagePartiel.email}</span>}
        </div>
        ...
        <div className="flex flex-col items-start mb-6">
          <div className="flex items-center gap-2">
            <input type="checkbox"  name="term"  id="term"  className="w-5 h-5 border-[#8692A6] rounded focus:ring-blue-500"  checked={formulaire.term}  onChange={(e) =>
        ...
        <button
             type="submit" className="w-full bg-green-500 hover:bg-green-600 text-white p-2 rounded-md">S'inscrire</button>
        <p className="text-sm text-center mt-4">
              Vous avez déjà un compte ?{' '}
          <a href="/" className="text-blue-500 hover:underline">Se connecter</a>
        </p>
